...
export function calculateFutureDate(hours: number): Date {
  const currentDate = new Date()
  const futureDate = new Date(currentDate.getTime() + hours * 60 * 60 * 1000) // Convert hours to milliseconds
  return futureDate
}
...
export const formatDateTime = (dateString: Date) => {
  ...
  return {
    dateTime: `${formattedDate} at ${formattedTime}`, // e.g., "Jan 21 at 23:45"
    dateOnly: formattedDate, // e.g., "Jan 21"
    timeOnly: formattedDateTime, // e.g., "23:45"
  }
}
